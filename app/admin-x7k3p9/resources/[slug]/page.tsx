...
import { fetchAdminResources } from "@/lib/adminResourcesDb";
...
export default async function AdminBoardPage({
  ...
  const items = rows.map((r: any) => {
    ...
    const dateLabel = ISSUE_BOARDS.has(slug)
      ? `발간 ${r.published_at ?? "-"}`
      : `게시 ${r.posted_at ?? "-"}`;
    ...
    const metaLine = `${dateLabel} · ${visibilityLabel} · 조회 ${
      r.views_count ?? 0
    }`;
    ...
    return {
      ...
      title: titleText,
      subtitle: subText,
      rightMeta: ISSUE_BOARDS.has(slug)
        ? guessIssue(titleText)
        : "",
      _raw: r,
    };
  });
