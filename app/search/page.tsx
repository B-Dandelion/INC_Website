...
function boardLabel(slug?: string | null, fallback?: string | null) {
  if (slug) {
    const match = RESOURCE_BOARDS.find((board) => board.slug === slug);
    if (match) return match.label;
  }
  return fallback || slug || "자료실";
}

function eventPeriod(start?: string | null, end?: string | null) {
    ...
    <main className="min-h-screen bg-[#F6F7F9]">
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-6xl px-5 py-11 md:px-6 md:py-14">
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-[-0.035em] text-slate-950 md:text-4xl">통합 검색</h1>
          <p className="mt-3 text-sm leading-6 text-slate-500 md:text-[15px]">
            자료실, 공지사항, 행사·이벤트를 한 번에 검색합니다.
          </p>
      ...
      <section className="mx-auto max-w-6xl px-5 py-10 md:px-6 md:py-14">
        {!q ? (
          <div className="border border-slate-200 bg-white px-6 py-14 text-center">
            ...
            <p className="mt-3 text-sm font-semibold text-slate-700">검색어를 입력해 주세요.</p>
            <p className="mt-1 text-sm text-slate-400">자료 제목, 공지 내용, 행사명과 장소 등을 찾을 수 있습니다.</p>
          </div>
        ) : total === 0 ? (
          <div className="border border-slate-200 bg-white px-6 py-14 text-center">
            <p className="text-base font-semibold text-slate-800">“{q}” 검색 결과가 없습니다.</p>
            <p className="mt-2 text-sm text-slate-400">검색어를 줄이거나 다른 표현으로 다시 검색해 보세요.</p>
          </div>
        ) : (
          <div className="space-y-10">
            <div className="flex items-baseline justify-between border-b border-slate-300 pb-3">
              <p className="text-sm text-slate-500">
                <span className="font-semibold text-slate-900">“{q}”</span> 검색 결과
              </p>
              <span className="text-xs font-semibold tabular-nums text-slate-400">총 {total}건</span>
            </div>
              ...
              <section aria-labelledby="search-resources">
                <div className="mb-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <FolderOpen className="h-4 w-4 text-[#174A7E]" />
                    <h2 id="search-resources" className="text-lg font-semibold text-slate-900">자료실</h2>
                  </div>
                  <span className="text-xs font-semibold tabular-nums text-slate-400">{resourceRows.length}건</span>
                </div>

                <div className="border border-slate-200 bg-white">
                  {resourceRows.map((resource, index) => {
                    const category = boardLabel(resource.boards?.slug, resource.boards?.title);
                    const title = (resource.title || resource.original_filename || "자료").trim();
                    ...
                    return (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className={`group flex items-start justify-between gap-5 px-5 py-4 transition hover:bg-slate-50 ${
                          index !== resourceRows.length - 1 ? "border-b border-slate-100" : ""
                        }`}
                      >
                        <div className="min-w-0">
                          <div className="mb-1.5 flex items-center gap-2 text-xs font-bold text-[#2B6CA3]">
                            <span>{category}</span>
                            <span className="font-normal text-slate-300">/</span>
                            <span className="font-medium text-slate-400">자료실</span>
                          </div>
                          <div className="text-[15px] font-semibold leading-6 text-slate-900 transition group-hover:text-[#174A7E]">
                            {title}
                          </div>
            ...
            {notices.length > 0 ? (
              <section aria-labelledby="search-notices">
                <div className="mb-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Bell className="h-4 w-4 text-[#174A7E]" />
                    <h2 id="search-notices" className="text-lg font-semibold text-slate-900">공지사항</h2>
                  </div>
                  <span className="text-xs font-semibold tabular-nums text-slate-400">{notices.length}건</span>
                </div>
                  ...
                  {notices.map((notice, index) => (
                    ...
                    >
                      <div className="min-w-0">
                        <div className="mb-1.5 flex items-center gap-2 text-xs font-bold text-[#2B6CA3]">
                          <span>공지사항</span>
                          {notice.pinned ? <span className="border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-[10px] text-slate-500">고정</span> : null}
                        </div>
            ...
            {events.length > 0 ? (
              <section aria-labelledby="search-events">
                <div className="mb-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <CalendarDays className="h-4 w-4 text-[#174A7E]" />
                    <h2 id="search-events" className="text-lg font-semibold text-slate-900">행사 · 이벤트</h2>
                  </div>
                  <span className="text-xs font-semibold tabular-nums text-slate-400">{events.length}건</span>
                </div>
                ...
                <div className="border border-slate-200 bg-white">
                  {events.map((event, index) => (
                    ...
                    >
                      <div className="min-w-0">
                        <div className="mb-1.5 text-xs font-bold text-[#2B6CA3]">행사 · 이벤트</div>
                        <div className="text-[15px] font-semibold leading-6 text-slate-900 transition group-hover:text-[#174A7E]">{event.title_ko}</div>
                        {event.summary_ko || event.content_ko ? (
                          <p className="mt-1 line-clamp-2 text-sm leading-6 text-slate-500">{compact(event.summary_ko || event.content_ko, 170)}</p>
                        ) : null}
                        <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-400">
                          <span className="inline-flex items-center gap-1.5 tabular-nums"><CalendarDays className="h-3.5 w-3.5" />{eventPeriod(event.event_date, event.period_end)}</span>
                          {event.location_ko ? <span className="inline-flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5" />{event.location_ko}</span> : null}
                        </div>
                      </div>
                      <ChevronRight className="mt-1 h-4 w-4 shrink-0 text-slate-300 transition group-hover:text-[#174A7E]" />
                    </Link>
