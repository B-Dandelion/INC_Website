...
function PosterContent({ asset, emptyText, alt }: { asset: any; emptyText: string; alt: string }) {
  const resourceId = asset?.resources?.id;

  if (!resourceId) {
    return <div className={styles.card}><div className={styles.muted}>{emptyText}</div></div>;
  }
...
export default async function ShortformContestPage({
  ...
  const tabs = [
    ...
    hasSubmissions ? { id: "materials", label: "응모작" } : null,
      ...
      <div className={styles.content}>
        ...
        {!selectedEventId ? (
          ...
          <>
                ...
                <div className={styles.posterGrid}>
                  {posterKo?.resources?.id ? (
                    <div className={styles.posterBlock}>
                      ...
                      <div className={styles.posterWrap}><div className={styles.posterInner}><PosterContent asset={posterKo} emptyText="" alt="국문 포스터" /></div></div>
                    </div>
                  ) : null}
                  {posterEn?.resources?.id ? (
                    <div className={styles.posterBlock}>
                      <div className={styles.posterLabel}>영문</div>
                      <div className={styles.posterWrap}><div className={styles.posterInner}><PosterContent asset={posterEn} emptyText="" alt="영문 포스터" /></div></div>
                    </div>
                  ) : null}
                </div>
              </section>
              ...
              <section id="winners" className={styles.section}>
                ...
                <div className={styles.winnerGrid}>
                  {awardDocs.map((w, idx) => {
                    ...
                    return (
                      <div key={`${doc?.id ?? idx}`} className={styles.winnerCard} style={{ gridTemplateColumns: "1fr", gap: 18, alignItems: "stretch" }}>
                        <div className={styles.winnerTop} style={{ justifyContent: "center", textAlign: "center" }}>
                        ...
                        <div className={styles.winnerBody} style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", alignItems: "center", gap: 20, width: "100%" }}>
                          {photoResource?.id ? (
                            <div className={styles.winnerPhoto} style={{ width: 160, height: 160, flex: "0 0 160px" }}>
                              <a href={`/api/resources/go?id=${photoResource.id}`} target="_blank" rel="noreferrer" style={{ display: "block", width: "100%", height: "100%" }}>
                                <img src={`/api/resources/go?id=${photoResource.id}`} alt={photoResource.title ?? "winner_photo"} />
                              </a>
                            </div>
                          ) : null}

                          <div className={styles.winnerInfo} style={{ flex: "1 1 360px", width: "100%", maxWidth: 640, minWidth: 0, textAlign: "center" }}>
                            {doc?.source_path && youtubeVideoId(doc.source_path) ? (
                              <div style={{ display: "grid", gap: 12, width: "100%" }}>
                                {w.item_title_ko || w.item_title_en ? (
                                  <div style={{ fontWeight: 700, color: "#263244", wordBreak: "break-word" }}>{w.item_title_ko ?? w.item_title_en}</div>
                                ) : null}
                                <YouTubePreview url={doc.source_path} title={w.item_title_ko ?? w.item_title_en ?? `${person} 수상 작품`} />
                                <a className={styles.winnerLink} href={doc.source_path} target="_blank" rel="noreferrer" style={{ justifySelf: "center" }}>유튜브에서 보기</a>
                              </div>
                            ) : doc?.id ? (
                              <a className={styles.winnerLink} href={`/api/resources/go?id=${doc.id}`} target="_blank" rel="noreferrer">응모작 열기</a>
                            ) : null}
                          </div>
                        </div>
                      </div>
                ...
                {otherPhotos.length > 0 ? (
                  <div className={styles.photoGrid}>
                    {otherPhotos.map((p, i) => {
                      ...
                      if (!rid) return null;
                      return (
                        <a key={`${rid}-${i}`} className={styles.photoThumb} href={`/api/resources/go?id=${rid}`} target="_blank" rel="noreferrer">
              ...
              <section id="materials" className={styles.section}>
                ...
                <div style={{ display: "grid", gap: 12 }}>
                  {submissions.map((asset, index) => {
                    ...
                    const label = (asset.resources?.title ?? asset.item_title_ko ?? asset.item_title_en ?? asset.resources?.original_filename ?? "응모작").trim() || "응모작";
                    const resourceId = asset.resources?.id;
                    ...
                    return (
                      <div key={`${resourceId}-${index}`} className={styles.card} style={{ display: "grid", gap: 12 }}>
                        <div style={{ fontWeight: 700, color: "#263244" }}>{label}</div>
                        {person ? <div className={styles.muted}>{person}</div> : null}
                        {videoId ? (
                          <>
                            <YouTubePreview url={sourcePath} title={label} />
                            <a className={styles.materialLink} href={sourcePath} target="_blank" rel="noreferrer">유튜브에서 보기</a>
                          </>
                        ) : (
                          <a className={styles.materialLink} href={`/api/resources/go?id=${resourceId}`} target="_blank" rel="noreferrer">응모작 열기</a>
                        )}
                      </div>
                    );
                  })}
            ...
            {!hasPoster && !hasWinners && !hasPhotos && !hasSubmissions ? (
