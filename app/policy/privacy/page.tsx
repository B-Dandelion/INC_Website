      ...
      <div className="grid gap-7">
        <section className={sectionClass}>
          ...
          <p className={bodyClass}>
            {en
              ? "INC processes information submitted through account registration to operate member accounts, review account approval, provide access to member-only resources, respond to account-related inquiries, and understand website usage for service operation and improvement."
              : "INC는 회원 계정 운영, 가입 승인 확인, 회원 전용 자료 접근 제공, 계정 관련 문의 대응 및 웹사이트 운영·개선을 위한 이용 현황 파악을 위해 필요한 정보를 처리합니다."}
          </p>
        </section>
        ...
        <section className={sectionClass}>
          ...
          <div className={bodyClass}>
            <ul className="list-disc space-y-1.5 pl-5">
              ...
              <li>{en ? "Anonymous usage statistics may include a randomly generated visitor identifier, page path, device type, referring domain, search terms, and resource-use events. IP addresses are not stored in the INC analytics event table." : "익명 이용 통계에는 무작위로 생성된 방문 식별자, 방문 경로, 기기 유형, 유입 도메인, 검색어 및 자료 이용 이벤트가 포함될 수 있습니다. INC의 이용 통계 이벤트 테이블에는 IP 주소를 저장하지 않습니다."}</li>
              <li>{en ? "Technical information such as access records may also be generated in infrastructure logs while the service is used." : "서비스 이용 과정에서 접속기록 등 기술 정보가 운영 인프라의 시스템 로그에 별도로 생성될 수 있습니다."}</li>
            </ul>
          </div>
        </section>
        ...
        <section className={sectionClass}>
          ...
          <p className={bodyClass}>
            {en
              ? "Account information is retained as necessary for account operation. Anonymous usage statistics are retained only as needed for website operation and analysis. If information no longer needs to be retained, or if deletion is requested and there is no legal reason to retain it, the information should be deleted in accordance with the applicable operating policy and law. Requests regarding personal information can be sent to the contact address below."
              : "회원정보는 계정 운영에 필요한 범위에서 보관되며, 익명 이용 통계는 웹사이트 운영 및 분석에 필요한 범위에서 보관됩니다. 정보 보관 필요가 없어지거나 이용자가 삭제를 요청하고 별도의 법적 보관 사유가 없는 경우에는 운영정책 및 관계 법령에 따라 삭제 대상이 됩니다. 개인정보 관련 요청은 아래 연락처로 접수할 수 있습니다."}
          </p>
        </section>
        ...
        <section className={sectionClass}>
          ...
          <p className={bodyClass}>
            {en
              ? "The website uses external cloud services for technical operation, including Supabase for authentication, database functions, and internal anonymous usage statistics; Vercel for web hosting, deployment, and Web Analytics; and Cloudflare R2 for file storage. Information required for each function may be technically processed by these services as part of providing the website."
              : "본 웹사이트는 기술적 운영을 위해 외부 클라우드 서비스를 사용합니다. 인증·데이터베이스 및 내부 익명 이용 통계에는 Supabase, 웹 호스팅·배포 및 Web Analytics에는 Vercel, 자료 파일 저장에는 Cloudflare R2가 사용되며, 웹사이트 제공에 필요한 정보가 각 서비스에서 기술적으로 처리될 수 있습니다."}
          </p>
        </section>
