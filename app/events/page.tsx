    ...
    <main className="min-h-screen bg-[#F6F7F9]">
      ...
      <section className="mx-auto max-w-6xl px-5 py-10 md:px-6 md:py-14">
        <div className="flex items-end justify-between border-b border-slate-300 pb-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">전체 이벤트</h2>
            <p className="mt-1 text-sm text-slate-500">최근 일정순으로 표시합니다.</p>
          </div>
          <span className="text-xs font-medium tabular-nums text-slate-400">{events.length} items</span>
        </div>

        {events.length === 0 ? (
          <div className="mt-5 border border-slate-200 bg-white px-5 py-14 text-center text-sm text-slate-400">
            등록된 이벤트가 없습니다.
          </div>
        ) : (
          <div className="mt-5 border border-slate-200 bg-white">
            {events.map((event, index) => {
              ...
              const ended = Boolean(end && end < today);
              return (
                <Link
                  key={event.id}
